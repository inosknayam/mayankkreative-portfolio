...
const Header = () => {
    ...
    return (
        <header className="h-[90px] flex items-center justify-between fixed top-0 left-0 right-0 z-50 px-8 md:px-12 backdrop-blur-md bg-white/5 dark:bg-black/5 border-b border-white/10 transition-all duration-300">
            {/* Logo */}
            <Link href="/" className="w-[90px] h-[90px] flex justify-center items-center text-accent text-[40px] font-semibold transition-all duration-400 hover:brightness-110 group">
                <span className="relative flex justify-center items-center transition-transform duration-400 group-hover:scale-115">
                    M
                    <span className="absolute left-3 top-1/2 w-[7px] h-[7px] bg-accent rounded-full"></span>
                </span>
            </Link>

            {/* Navigation (Desktop) */}
            <div className={`fixed inset-0 backdrop-blur-md bg-white/10 dark:bg-black/10 md:bg-transparent md:backdrop-blur-none z-40 flex flex-col justify-center items-center transition-all duration-300 ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} md:static md:opacity-100 md:pointer-events-auto md:flex-row md:justify-end md:h-auto`}>
            ...
            <div className="flex items-center gap-6 md:gap-0">
                ...
                {mounted && (
                    <button
                        ...
                        className="w-[90px] h-[90px] flex justify-center items-center text-foreground hover:text-accent transition-colors border-l border-white/10 md:mr-0"
                    >
                        {theme === 'dark' ? <FiSun className="w-6 h-6" /> : <FiMoon className="w-6 h-6" />}
                    </button>
                )}

                <Link href="/contact" className="w-[90px] h-[90px] flex justify-center items-center text-accent transition-all duration-400 hover:brightness-110 group hidden md:flex">
